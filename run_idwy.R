...
# d1=read_csv('idwy_WhichDatesSpatialSWE.csv',skip=1,col_names = 'dte')
# d2=read_csv('idwy_moredates.csv',skip = 1,col_names = 'dte')
#
# bind_rows(d1,d2) %>%
# 	mutate(dte=as.Date(strptime(dte,'%d-%b-%Y'))) %>%
# 	arrange(dte) %>%
# 	write_csv('idwy_alldates_ordered.csv')
